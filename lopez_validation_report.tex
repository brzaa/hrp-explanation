...
\section{The Missing Simulation Evidence}
\label{sec:simulation_gap}
\subsection{Why Simulation Matters}
Simulation allows us to control the data-generating process, perturb key assumptions, and observe whether the strategy holds up. For HRP, relevant simulations include:
\begin{itemize}
    \item \textbf{Correlation Regime Shifts}: Simulate markets where correlation structures break down (e.g., crisis clusters) to see if HRP's hierarchy adapts.
    \item \textbf{Volatility Shocks}: Inject sudden variance spikes to test recursive bisection sensitivity.
    \item \textbf{Factor Rotations}: Vary underlying factor exposures to evaluate whether HRP's clustering captures economic structure or merely statistical noise.
\end{itemize}
The absence of these simulations leaves the validation half-finished according to López de Prado's schema.

\subsection{Potential Simulation Framework}
To align with López de Prado's guidance, a future validation module should:
\begin{enumerate}
    \item Generate synthetic return series using a multivariate regime-switching process calibrated to historical moments (means, variances, correlations).
    \item Introduce structural breaks in correlations and volatilities to model crises.
    \item Apply HRP, 1/N, Minimum Variance, and Risk Parity to these simulated paths.
    \item Record distributional statistics (median, tail quantiles) of portfolio performance and turnover.
\end{enumerate}
Such a framework would reveal whether HRP's hierarchy is a genuine robustness enhancer or a sample-specific artefact.

\subsection{Challenges and Caveats}
Simulations must respect stylised facts: heavy tails, volatility clustering, and cross-asset dependencies. López de Prado warns that naive Gaussian simulations produce misleading comfort. Implementing realistic simulations is non-trivial but essential. Without them, claims of robustness lack foundation.
